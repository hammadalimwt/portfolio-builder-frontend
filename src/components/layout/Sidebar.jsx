...
export default function Sidebar({ isOpen, setIsOpen }) {
  ...
  const navItems = [
    ...
    { name: 'My Portfolios', path: '/dashboard/portfolios', icon: FolderOpen },
    { name: 'Create Portfolio', path: '/dashboard/create', icon: Plus },
  ...
  return (
    <aside className={`pb-sidebar ${isOpen ? 'pb-sidebar-open' : 'pb-sidebar-collapsed'}`}>
      <style>{`
        .pb-sidebar {
          position: fixed;
          top: 0;
      ...
      <div className="pb-sidebar-footer">
        ...
        <div className="pb-sidebar-user-info">
          <span className="pb-sidebar-user-name truncate">{user?.name || 'SaaS Builder'}</span>
          <span className="pb-sidebar-user-email truncate">{user?.email || 'user@example.com'}</span>
        </div>
      </div>
    </aside>
  );
